    ...
    for attempt in 1..=MAX_RETRIES {
        ...
        let response = match response {
            ...
            Err(e) if attempt == MAX_RETRIES => {
                return Err(anyhow!("HTTP request failed after {} attempts: {}", MAX_RETRIES, e));
            }
            Err(_) => {
        ...
        let text = match response.text() {
            Ok(t) => t,
            Err(e) if attempt == MAX_RETRIES => {
                return Err(anyhow!("Failed to read response text after {} attempts: {}", MAX_RETRIES, e));
            }
            Err(_) => {
                // Exponential backoff: 1s, 2s, 4s, 8s, 16s
                let delay_secs = 2_u64.pow(attempt - 1);
        ...
        let json: Value = match serde_json::from_str(&text) {
            Ok(j) => j,
            Err(e) if attempt == MAX_RETRIES => {
                return Err(anyhow!("Failed to parse JSON after {} attempts: {}", MAX_RETRIES, e));
            }
            Err(_) => {
                // Exponential backoff: 1s, 2s, 4s, 8s, 16s
                let delay_secs = 2_u64.pow(attempt - 1);
        ...
        let download_url = match download_url {
            ...
            None if attempt == MAX_RETRIES => {
                return Err(anyhow!("No valid bulk download link found after {} attempts", MAX_RETRIES));
            }
            None => {
                // Exponential backoff: 1s, 2s, 4s, 8s, 16s
                let delay_secs = 2_u64.pow(attempt - 1);
        ...
        match http_get(download_url, None, None, 3, DOWNLOAD_TIMEOUT).await {
            ...
            Err(e) if attempt == MAX_RETRIES => {
                return Err(anyhow!("Failed to download zip file after {} attempts: {}", MAX_RETRIES, e));
            }
            Err(_) => {
                // Exponential backoff: 1s, 2s, 4s, 8s, 16s
                let delay_secs = 2_u64.pow(attempt - 1);
